...
if __name__ == '__main__':
    ...
    for metric in ['Pearsonr', 'Spearmanr']:
        ...
        ncols = 2
        nrows = math.ceil(len(model_pairs) / ncols)
        colsize = subplot_size
        rowsize = subplot_size
        fig, axs = plt.subplots(nrows, ncols, figsize=(ncols*colsize, nrows*rowsize), sharex=True, sharey=True)
        ...
        plt.tight_layout()
        plot_name = f'comparison_plots_hcnn_finetuned_biopython_{metric.lower()}'
        plt.savefig(f'{plot_name}.png')
        plt.close()
    ...
    for metric in ['Pearsonr', 'Spearmanr']:
        ...
        plt.tight_layout()
        plot_name = f'comparison_plots_hcnn_finetuned_pyrosetta_{metric.lower()}'
        plt.savefig(f'{plot_name}.png')
        plt.close()
    ...
    for metric in ['Pearsonr', 'Spearmanr']:
        ...
        plt.tight_layout()
        plot_name = f'comparison_plots_hcnn_finetuned_biopython_vs_pyrosetta_{metric.lower()}'
        plt.savefig(f'{plot_name}.png')
        plt.close()
    ...
    model_pairs_row_3 = [('HCNN_biopython_proteinnet_extra_mols_0p50_finetuned_with_rosetta_ddg_with_0p00_all', 'proteinmpnn_v_48_002'), ('HCNN_biopython_proteinnet_extra_mols_0p50_finetuned_with_rosetta_ddg_with_0p00_all', 'proteinmpnn_v_48_030')]

    all_pairs = [model_pairs_row_1, model_pairs_row_2, model_pairs_row_3]
    ...
    for metric in ['Pearsonr', 'Spearmanr']:
        ...
        ncols = max([len(pairs) for pairs in all_pairs])
        nrows = len(all_pairs)
        colsize = subplot_size
        rowsize = subplot_size
        fig, axs = plt.subplots(nrows, ncols, figsize=(ncols*colsize, nrows*rowsize), sharex=True, sharey=True)

        for row, model_pairs in enumerate(all_pairs):
