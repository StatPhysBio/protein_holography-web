...
for pdbid, chain in zip(['AF-Q9ES00-F1-model_v4'],
                        ['A']):
    df_copy = df.copy()
...
for pdbid, chain in zip(['rank_1_model_1_ptm_seed_0_unrelaxed'],
                        ['A']):
    df_copy = df.copy()

    # shift resnums by minus 1071, remove the ones with negative resnums

    new_resnums = np.array([int(mutant[1:-1])-1071 for mutant in df_copy['mutant']])
