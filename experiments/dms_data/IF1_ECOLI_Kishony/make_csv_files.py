...
for pdbid, chain in zip(['AF-P69222-F1-model_v4'],
                        ['A']):
    df_copy = df.copy()
...
for pdbid, chain in zip(['1ah9'],
                        ['A']):
    df_copy = df.copy()

    # shift resnums by minus one, remove the ones with negative resnums

    new_resnums = np.array([int(mutant[1:-1])-1 for mutant in df_copy['mutant']])
