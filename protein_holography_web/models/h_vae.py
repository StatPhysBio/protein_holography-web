...
from protein_holography_pytorch.so3.functional import make_vec
from protein_holography_pytorch.utils.loss_functions import *
...
class H_VAE(torch.nn.Module):
    ...
    def forward(self, x: Dict[int, Tensor], x_vec: Optional[Tensor] = None, frame: Optional[Tensor] = None):
        ...
        # print('Is x nan: ', torch.any(torch.isnan(x_vec)).item(), file=sys.stderr)
        # print('Is x inf: ', torch.any(torch.isinf(x_vec)).item(), file=sys.stderr)
        ...
        # print('Is z nan: ', torch.any(torch.isnan(z)).item(), file=sys.stderr)
        # print('Is z inf: ', torch.any(torch.isinf(z)).item(), file=sys.stderr)
        # print('Is learned frame nan: ', torch.any(torch.isnan(learned_frame)).item(), file=sys.stderr)
        # print('Is learned frame inf: ', torch.any(torch.isinf(learned_frame)).item(), file=sys.stderr)

        # if torch.any(torch.isnan(learned_frame)).item():
        #     nan_idxs = torch.where(torch.isnan(learned_frame))[0]
        #     print('Num nans: ', nan_idxs.shape[0], file=sys.stderr)
        #     print(x[1][nan_idxs], file=sys.stderr)
        #     np.save('faulty_data.npy', x_vec.detach().cpu().numpy())
        #     np.save('faulty_frames.npy', learned_frame.detach().cpu().numpy())
        #     exit(1)
        ...
        # print('Is x_reconst nan: ', torch.any(torch.isnan(x_reconst_vec)).item(), file=sys.stderr)
        # print('Is x_reconst inf: ', torch.any(torch.isinf(x_reconst_vec)).item(), file=sys.stderr)
        ...
        # if torch.any(torch.isnan(x_reconst_loss)):
        #     print(torch.any(torch.isnan(x_vec)))
        #     print(torch.any(torch.isnan(x_reconst_vec)))
        #     print(torch.any(torch.isnan(z)))
        #     print(torch.any(torch.isnan(frame)))

        #     print(torch.any(torch.isinf(x_vec)))
        #     print(torch.any(torch.isinf(x_reconst_vec)))
        #     print(torch.any(torch.isinf(z)))
        #     print(torch.any(torch.isinf(frame)))



        # print('Is loss nan: ', torch.any(torch.isnan(x_reconst_loss)).item(), file=sys.stderr)
        # print('Is loss inf: ', torch.any(torch.isinf(x_reconst_loss)).item(), file=sys.stderr)
